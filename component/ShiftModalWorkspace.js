...
export default function ShiftModalWorkspace({
    setShiftVisible,
    seen,
    delivered,
    shiftId,
    ...
    const shiftGroup = async () => {
        await axios
            .post('https://api.sheikhanigroup.com/shiftWorkspace', {
                roomid: shiftId,
            })
            .then(res => {
                setRefresh(!refresh);
                Alert.alert('', 'External Group Transfered To All Groups Successfully');
                setShiftVisible(false);
            })
            .catch(err => {
