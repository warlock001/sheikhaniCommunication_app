...
const Announcements = () => {
  ...
  const AnnouncementComponent = ({ id, title, description, date, user }) => {
    ...
    return (
      <Pressable
        onPress={() => navigation.navigate('AnnouncementPreview', { id: id, title: title, description: description, date: date, user: user })}
        style={{
          ...
          <View style={{ padding: 10 }}>
            ...
            {Array.isArray(allAnnouncements) && allAnnouncements.length > 0 ? (
              <FlatList
                ...
                renderItem={({ item }) => (
                  <AnnouncementComponent id={item._id} title={item.title} description={item.description} date={item.createdAt} user={item.user} />
                )}
                keyExtractor={item => item.user}
              />
            ) : (
              <View style={styles.chatemptyContainer}>
                <Text style={styles.chatemptyText}>No Announcements To View</Text>
              </View>
