...
const Profile = () => {
  ...
  useEffect(() => {
    async function getProfilePictureURL() {
      ...
      axios
        .get(`http://api.sheikhanigroup.com:3001/files/${profilepicture}/true`)
        .then(res => {
  ...
  async function sendData() {
    if (!image) {
      ...
      await axios({
        timeout: 20000,
        method: 'POST',
        url: `http://api.sheikhanigroup.com:3001/profilepicture`,
        data: form,
        headers: {
        ...
        .then(async res => {
          ...
          await axios
            .get(`http://api.sheikhanigroup.com:3001/files/${res.data.id}/true`)
            .then(res => {
              setprofilepictureURL(
                `data:${res.headers['content-type']};base64,${res.data}`,
                toggleEditModal(),
